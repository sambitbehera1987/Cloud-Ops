...
resource "google_compute_global_address" "default" {
  ...
  description        = "App Loadbalancer IP"
...
resource "google_compute_target_https_proxy" "default" {
  ...
  ssl_certificates = compact(concat(var.ssl_certificates, google_compute_managed_ssl_certificate.default.*.self_link, ), )
...
resource "google_compute_managed_ssl_certificate" "default" {
  ...
  project  = var.project
  count    = var.ssl && length(var.managed_ssl_certificate_domains) > 0 && ! var.use_ssl_certificates ? 1 : 0
  description    = "SSL certificate for App Servers"
  name =  "${var.name}-cert" 
...
resource "google_compute_backend_service" "default" {
  ...
  name = each.value.name == "" ? "${var.name}-backend-${each.key}" : each.value.name
...
resource "google_compute_health_check" "default" {
  ...
  for_each = var.backends
  project  = var.project
  name = lookup(each.value["health_check"], "name") == "" ? "${var.name}-backend-${each.key}" : lookup(each.value["health_check"], "name")
  ...
  dynamic "http_health_check" {
    for_each = lookup(each.value["health_check"], "protocol") == "HTTP" ? [
      {
  ...
  dynamic "https_health_check" {
    for_each = lookup(each.value["health_check"], "protocol") == "HTTPS" ? [
      {
        host         = lookup(each.value["health_check"], "host", null)
        request_path = lookup(each.value["health_check"], "request_path", null)
        port         = lookup(each.value["health_check"], "port", null)
  ...
  dynamic "http2_health_check" {
    for_each = lookup(each.value["health_check"], "protocol") == "HTTP2" ? [
      {
        host         = lookup(each.value["health_check"], "host", null)
        request_path = lookup(each.value["health_check"], "request_path", null)
        port         = lookup(each.value["health_check"], "port", null)
  ...
  dynamic "tcp_health_check" {
    for_each = lookup(each.value["health_check"], "protocol") == "TCP" ? [
      {
        port         = lookup(each.value["health_check"], "port", null)
      }
    ] : []
